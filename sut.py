...
class sut(object):
    ...
    def act1(self):
        ...
        // Test adding a task
        '''
        self.__test.append(('''// Test adding a task ''',self.guard1,self.act1))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test adding a task '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard2(self):
        '''
        add_task : (string, string?)
        '''
        return (add_task : (string, string?))
    
    def act3(self):
        '''
        // Test removing a task
        '''
        self.__test.append(('''// Test removing a task ''',self.guard3,self.act3))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test removing a task '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard4(self):
        ...
        remove_task : (int)
        '''
        return (remove_task : (int))
    
    def act5(self):
        '''
        // Test marking a task as done
        '''
        self.__test.append(('''// Test marking a task as done ''',self.guard5,self.act5))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test marking a task as done '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard6(self):
        ...
        mark_as_done : (int)
        '''
        return (mark_as_done : (int))
    
    def act7(self):
        '''
        // Test viewing all tasks
        '''
        self.__test.append(('''// Test viewing all tasks ''',self.guard7,self.act7))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test viewing all tasks '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def act9(self):
        ...
        '''
        self.__test.append(('''// Test viewing pending tasks ''',self.guard9,self.act9))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test viewing pending tasks '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def act11(self):
        ...
        '''
        self.__test.append(('''// Test viewing completed tasks ''',self.guard11,self.act11))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test viewing completed tasks '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def act13(self):
        '''
        // Test setting due date for a task
        '''
        self.__test.append(('''// Test setting due date for a task ''',self.guard13,self.act13))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test setting due date for a task '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard14(self):
        '''
        set_due_date : (int, string)
        '''
        return (set_due_date : (int, string))
    
    def act15(self):
        '''
        // Test searching for tasks
        '''
        self.__test.append(('''// Test searching for tasks ''',self.guard15,self.act15))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test searching for tasks '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard16(self):
        ...
        search_tasks : (string)
        '''
        return (search_tasks : (string))
    
    def act17(self):
        '''
        // Test saving tasks to a file
        '''
        self.__test.append(('''// Test saving tasks to a file ''',self.guard17,self.act17))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test saving tasks to a file '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def guard18(self):
        '''
        save_tasks_to_file : (string)
        '''
        return (save_tasks_to_file : (string))
    
    def act19(self):
        '''
        // Test loading tasks from a file
        '''
        self.__test.append(('''// Test loading tasks from a file ''',self.guard19,self.act19))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
            print('ACTION:',self.prettyName('''// Test loading tasks from a file '''))
        if self.__collectCov: self.__cov.start()
        try: test_before_each(self)
        except: pass
        try:
    ...
    def act20(self):
        ...
        none
        '''
        self.__test.append(('''none ''',self.guard20,self.act20))
        self.__raised = None
        self.__refRaised = None
        if self.__verboseActions:
            __bV = {}
    ...
    def guard20(self):
        '''
        load_tasks_from_file : (string)
        '''
        return (load_tasks_from_file : (string))
    ...
    # END INITIALIZATION CODE
        self.__SUTName = """ToDoListManager"""
        self.__actions = []
        self.__names = {}
        ...
        self.__actionClasses.append('''// Test adding a task ''')
        self.__actionClasses.append('''add_task : (string, string?) -> none ''')
        self.__actionClasses.append('''// Test removing a task ''')
        self.__actionClasses.append('''remove_task : (int) -> none ''')
        self.__actionClasses.append('''// Test marking a task as done ''')
        self.__actionClasses.append('''mark_as_done : (int) -> none ''')
        self.__actionClasses.append('''// Test viewing all tasks ''')
        self.__actionClasses.append('''view_all_tasks : -> none ''')
        self.__actionClasses.append('''// Test viewing pending tasks ''')
        self.__actionClasses.append('''view_pending_tasks : -> none ''')
        self.__actionClasses.append('''// Test viewing completed tasks ''')
        self.__actionClasses.append('''view_completed_tasks : -> none ''')
        self.__actionClasses.append('''// Test setting due date for a task ''')
        self.__actionClasses.append('''set_due_date : (int, string) -> none ''')
        self.__actionClasses.append('''// Test searching for tasks ''')
        self.__actionClasses.append('''search_tasks : (string) -> none ''')
        self.__actionClasses.append('''// Test saving tasks to a file ''')
        self.__actionClasses.append('''save_tasks_to_file : (string) -> none ''')
        self.__actionClasses.append('''// Test loading tasks from a file ''')
        self.__actionClasses.append('''load_tasks_from_file : (string) -> none ''')
        self.__dependencies = {}
        self.__dependencies['''include "main.py" '''] = []
        self.__dependencies['''// Test adding a task '''] = []
        self.__dependencies['''add_task : (string, string?) -> none '''] = []
        self.__dependencies['''// Test removing a task '''] = []
        self.__dependencies['''remove_task : (int) -> none '''] = []
        self.__dependencies['''// Test marking a task as done '''] = []
        self.__dependencies['''mark_as_done : (int) -> none '''] = []
        self.__dependencies['''// Test viewing all tasks '''] = []
        self.__dependencies['''view_all_tasks : -> none '''] = []
        self.__dependencies['''// Test viewing pending tasks '''] = []
        self.__dependencies['''view_pending_tasks : -> none '''] = []
        self.__dependencies['''// Test viewing completed tasks '''] = []
        self.__dependencies['''view_completed_tasks : -> none '''] = []
        self.__dependencies['''// Test setting due date for a task '''] = []
        self.__dependencies['''set_due_date : (int, string) -> none '''] = []
        self.__dependencies['''// Test searching for tasks '''] = []
        self.__dependencies['''search_tasks : (string) -> none '''] = []
        self.__dependencies['''// Test saving tasks to a file '''] = []
        self.__dependencies['''save_tasks_to_file : (string) -> none '''] = []
        self.__dependencies['''// Test loading tasks from a file '''] = []
        self.__dependencies['''load_tasks_from_file : (string) -> none '''] = []
        self.__orderings = {}
        self.__okExcepts = {}
        self.__preCode = {}
        self.__refCode = {}
        ...
        self.__actions.append(('''// Test adding a task ''',self.guard1,self.act1))
        self.__names['''// Test adding a task '''] = ('''// Test adding a task ''',self.guard1,self.act1)
        self.__actionClass['''// Test adding a task '''] = '''// Test adding a task '''
        self.__orderings['''// Test adding a task '''] = 2
        self.__okExcepts['''// Test adding a task '''] = ''''''
        
        self.__actions.append(('''none ''',self.guard2,self.act2))
        self.__names['''none '''] = ('''none ''',self.guard2,self.act2)
        self.__actionClass['''none '''] = '''add_task : (string, string?) -> none '''
        self.__orderings['''none '''] = 3
        self.__okExcepts['''none '''] = ''''''
        
        self.__actions.append(('''// Test removing a task ''',self.guard3,self.act3))
        self.__names['''// Test removing a task '''] = ('''// Test removing a task ''',self.guard3,self.act3)
        self.__actionClass['''// Test removing a task '''] = '''// Test removing a task '''
        self.__orderings['''// Test removing a task '''] = 4
        self.__okExcepts['''// Test removing a task '''] = ''''''
        
        self.__actions.append(('''none  ''',self.guard4,self.act4))
        self.__names['''none  '''] = ('''none  ''',self.guard4,self.act4)
        self.__actionClass['''none  '''] = '''remove_task : (int) -> none '''
        self.__orderings['''none  '''] = 5
        self.__okExcepts['''none  '''] = ''''''
        self.__refCode['''none  '''] = []
        self.__refCode['''none  '''].append('''none ''')
        
        self.__actions.append(('''// Test marking a task as done ''',self.guard5,self.act5))
        self.__names['''// Test marking a task as done '''] = ('''// Test marking a task as done ''',self.guard5,self.act5)
        self.__actionClass['''// Test marking a task as done '''] = '''// Test marking a task as done '''
        self.__orderings['''// Test marking a task as done '''] = 6
        self.__okExcepts['''// Test marking a task as done '''] = ''''''
        
        self.__actions.append(('''none   ''',self.guard6,self.act6))
        self.__names['''none   '''] = ('''none   ''',self.guard6,self.act6)
        self.__actionClass['''none   '''] = '''mark_as_done : (int) -> none '''
        self.__orderings['''none   '''] = 7
        self.__okExcepts['''none   '''] = ''''''
        self.__refCode['''none   '''] = []
        self.__refCode['''none   '''].append('''none ''')
        
        self.__actions.append(('''// Test viewing all tasks ''',self.guard7,self.act7))
        self.__names['''// Test viewing all tasks '''] = ('''// Test viewing all tasks ''',self.guard7,self.act7)
        self.__actionClass['''// Test viewing all tasks '''] = '''// Test viewing all tasks '''
        self.__orderings['''// Test viewing all tasks '''] = 8
        self.__okExcepts['''// Test viewing all tasks '''] = ''''''
        
        self.__actions.append(('''none    ''',self.guard8,self.act8))
        self.__names['''none    '''] = ('''none    ''',self.guard8,self.act8)
        self.__actionClass['''none    '''] = '''view_all_tasks : -> none '''
        self.__orderings['''none    '''] = 9
        self.__okExcepts['''none    '''] = ''''''
        self.__refCode['''none    '''] = []
        self.__refCode['''none    '''].append('''none ''')
        
        self.__actions.append(('''// Test viewing pending tasks ''',self.guard9,self.act9))
        self.__names['''// Test viewing pending tasks '''] = ('''// Test viewing pending tasks ''',self.guard9,self.act9)
        self.__actionClass['''// Test viewing pending tasks '''] = '''// Test viewing pending tasks '''
        self.__orderings['''// Test viewing pending tasks '''] = 10
        self.__okExcepts['''// Test viewing pending tasks '''] = ''''''
        
        self.__actions.append(('''none     ''',self.guard10,self.act10))
        self.__names['''none     '''] = ('''none     ''',self.guard10,self.act10)
        self.__actionClass['''none     '''] = '''view_pending_tasks : -> none '''
        self.__orderings['''none     '''] = 11
        self.__okExcepts['''none     '''] = ''''''
        self.__refCode['''none     '''] = []
        self.__refCode['''none     '''].append('''none ''')
        
        self.__actions.append(('''// Test viewing completed tasks ''',self.guard11,self.act11))
        self.__names['''// Test viewing completed tasks '''] = ('''// Test viewing completed tasks ''',self.guard11,self.act11)
        self.__actionClass['''// Test viewing completed tasks '''] = '''// Test viewing completed tasks '''
        self.__orderings['''// Test viewing completed tasks '''] = 12
        self.__okExcepts['''// Test viewing completed tasks '''] = ''''''
        
        self.__actions.append(('''none      ''',self.guard12,self.act12))
        self.__names['''none      '''] = ('''none      ''',self.guard12,self.act12)
        self.__actionClass['''none      '''] = '''view_completed_tasks : -> none '''
        self.__orderings['''none      '''] = 13
        self.__okExcepts['''none      '''] = ''''''
        self.__refCode['''none      '''] = []
        self.__refCode['''none      '''].append('''none ''')
        
        self.__actions.append(('''// Test setting due date for a task ''',self.guard13,self.act13))
        self.__names['''// Test setting due date for a task '''] = ('''// Test setting due date for a task ''',self.guard13,self.act13)
        self.__actionClass['''// Test setting due date for a task '''] = '''// Test setting due date for a task '''
        self.__orderings['''// Test setting due date for a task '''] = 14
        self.__okExcepts['''// Test setting due date for a task '''] = ''''''
        
        self.__actions.append(('''none       ''',self.guard14,self.act14))
        self.__names['''none       '''] = ('''none       ''',self.guard14,self.act14)
        self.__actionClass['''none       '''] = '''set_due_date : (int, string) -> none '''
        self.__orderings['''none       '''] = 15
        self.__okExcepts['''none       '''] = ''''''
        self.__refCode['''none       '''] = []
        self.__refCode['''none       '''].append('''none ''')
        
        self.__actions.append(('''// Test searching for tasks ''',self.guard15,self.act15))
        self.__names['''// Test searching for tasks '''] = ('''// Test searching for tasks ''',self.guard15,self.act15)
        self.__actionClass['''// Test searching for tasks '''] = '''// Test searching for tasks '''
        self.__orderings['''// Test searching for tasks '''] = 16
        self.__okExcepts['''// Test searching for tasks '''] = ''''''
        
        self.__actions.append(('''none        ''',self.guard16,self.act16))
        self.__names['''none        '''] = ('''none        ''',self.guard16,self.act16)
        self.__actionClass['''none        '''] = '''search_tasks : (string) -> none '''
        self.__orderings['''none        '''] = 17
        self.__okExcepts['''none        '''] = ''''''
        self.__refCode['''none        '''] = []
        self.__refCode['''none        '''].append('''none ''')
        
        self.__actions.append(('''// Test saving tasks to a file ''',self.guard17,self.act17))
        self.__names['''// Test saving tasks to a file '''] = ('''// Test saving tasks to a file ''',self.guard17,self.act17)
        self.__actionClass['''// Test saving tasks to a file '''] = '''// Test saving tasks to a file '''
        self.__orderings['''// Test saving tasks to a file '''] = 18
        self.__okExcepts['''// Test saving tasks to a file '''] = ''''''
        
        self.__actions.append(('''none         ''',self.guard18,self.act18))
        self.__names['''none         '''] = ('''none         ''',self.guard18,self.act18)
        self.__actionClass['''none         '''] = '''save_tasks_to_file : (string) -> none '''
        self.__orderings['''none         '''] = 19
        self.__okExcepts['''none         '''] = ''''''
        self.__refCode['''none         '''] = []
        self.__refCode['''none         '''].append('''none ''')
        
        self.__actions.append(('''// Test loading tasks from a file ''',self.guard19,self.act19))
        self.__names['''// Test loading tasks from a file '''] = ('''// Test loading tasks from a file ''',self.guard19,self.act19)
        self.__actionClass['''// Test loading tasks from a file '''] = '''// Test loading tasks from a file '''
        self.__orderings['''// Test loading tasks from a file '''] = 20
        self.__okExcepts['''// Test loading tasks from a file '''] = ''''''
        
        self.__actions.append(('''none          ''',self.guard20,self.act20))
        self.__names['''none          '''] = ('''none          ''',self.guard20,self.act20)
        self.__actionClass['''none          '''] = '''load_tasks_from_file : (string) -> none '''
        self.__orderings['''none          '''] = 21
        self.__okExcepts['''none          '''] = ''''''
        self.__refCode['''none          '''] = []
        self.__refCode['''none          '''].append('''none ''')
